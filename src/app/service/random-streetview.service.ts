...
export class RandomStreetviewService{
  ...
  async getRandomLocations(nLocations: any, onLocation = (...args: any) => 0) {
      let get = async () => {
...
class StreetView {
  ...
  async randomValidLocation({
      ...
      let pixelCounts = {count: 0, indices: <number[]>[]};
  ...
  containsLocation(location: any, polygon: any) {
    if (polygon.length === 0)
      return true;
  ...
  async waitSleep(time: number) {
      return new Promise(resolve => {
          setTimeout(resolve, time);
      });
  }

  async randomValidTile(endZoom: number, type: string, chosenTile = {x: 0, y: 0, zoom: 0}, startZoom = chosenTile.zoom): Promise<any> {
      ...
      let validTiles = subTiles
          ...
          .filter((tile: any) => this.tileIntersectsMap(tile.x, tile.y, tile.zoom));
      ...
      let shuffleFun = this.distribution === 'uniform' ?
          (array: any) => this.shuffle(array) :
          (array: any) => this.shuffleWeighted(array, item => item.coverage[chosenTile.zoom + 1 <= photoSphereZoomLevel ? 'both' : type]);
  ...
  tileEquals(tileA: any, tileB: any) {
      return (tileA.x === tileB.x && tileA.y === tileB.y && tileA.zoom === tileB.zoom);
  }

  getTileCornerCoordinates(tileX: any, tileY: any, zoom: number) {
      return [
  ...
  tileIntersectsMap(tileX: any, tileY: any, zoom: number) {
      ...
      this.polygon.getPaths().forEach((path: any) => {
  ...
  async getSubTiles(x: number, y: number, zoom: number): Promise<any> {
  ...
  async getTileGrid(startX: number, endX: number, startY: number, endY: number, zoom: number) {
      let tasks = [];
  ...
  tilePixelToCoordinate(tileX: number, tileY: number, zoom: number, pixelX: number, pixelY: number) {
      tileX += pixelX / 256;
      tileY += pixelY / 256;
  ...
  coordinateToTile(coordinate: any, zoom: number) {
  ...
  async getTileImage(x: number, y: number, zoom: number) {
  ...
  async getTile(x: number, y: number, zoom: number) {
      return new Promise(async resolve => {
  ...
  isTileFullyContainedInMap(tileX: number, tileY: number, zoom: number) {
  ...
  async getTileCoverage(tileX: number, tileY: number, zoom: number, img: any) {
  ...
  coverageCacheContains(x: number, y: number, zoom: number) {
  ...
  getCoverageCache(x: number, y: number, zoom: number) {
      let id = this.cacheKey;
  ...
  setCoverageCache(x: number, y: number, zoom: number, value: any) {
      let id = this.cacheKey;
  ...
  shuffleWeighted(array: any, weightField = (item: any) => item.weight) {
      if (array.length === 0)
          return array;
      let result = [];
